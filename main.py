...
def create_table(conn,cur):
    cur.execute(
        """
        Create Table if not exist candidates (
            candidate_id VARCHAR(255) PRIMARY KEY,
            candidate_name VARCHAR(255),
            party_affiliation VARCHAR(255),
            biography TEXT,
            camapaign_platform TEXT,
            photo_url TEXT
        )
"""
    )
...
if __name__ == "main":
    try:
        conn=psycopg2.connect("host=host.docker.internal dbname=voting user=postgres password=postgres")
        cur=conn.cursor()
